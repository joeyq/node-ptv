...
PTV.prototype.healthCheck = function () {
    this._callAPI(
        '/healthcheck',
        function (err, res) {
            console.log(res);
...
PTV.prototype.stopsNearby = function (latitude, longitude, cb) {
    this._callAPI(
        ...
        function (err, res) {
            ...
            return cb(null, res.map(function (s) {
                return s.result;
            }));
        }
    );
...
PTV.prototype.linesByMode = function (mode, name, cb) {
    this._callAPI(
        util.format('/lines/mode/%d?name=%s', mode, name),
        cb
    );
};

PTV.prototype.broadNextDepartures = function (mode, stop, limit, date, cb) {
    var modeID = PTV.modeName.indexOf(mode);
    if (!date)
        date = new Date();
    this._callAPIutc(
...
PTV.disruptionModes = ['general', 'metro-bus', 'metro-train', 'metro-tram',
    'regional-bus', 'regional-coach', 'regional-train'];

PTV.prototype.disruptions = function (mode, cb) {
    var modes = "";
    ...
    this._callAPI(
        util.format('/disruptions/modes/%s', modes),
        cb
    );
};
